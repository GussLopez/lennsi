...
export default function SignForm({ restaurantId, signId }: SignFormProps) {
  const query = useQuery({
    queryKey: ["signs", restaurantId], queryFn: () => fetchSigns(restaurantId)
  })
  ...
  if (!query.data.canManage) return <FormMessage
    message="No tienes permiso para administrar carteles."
  />
  ...
  return (
    <SignFormFields
      key={`${restaurantId}:${signId ?? "new"}`}
      restaurantId={restaurantId}
      signId={signId}
      token={sign?.token} initialValues={{
        label: sign?.label ?? "",
        businessName: sign?.business_name ?? "",
        destinationUrl: sign?.destination_url ?? "",
        isActive: sign?.is_active ?? true,
      }}
    />
  )
}
  ...
  return (
    <form className="space-y-6" onSubmit={handleSubmit((values) => mutation.mutate(values))} noValidate>
        ...
        <div className="grid gap-5 p-5 sm:grid-cols-2 sm:p-6">
          ...
          <div className="space-y-2 sm:col-span-2">
            ...
            <Input
              ...
              aria-describedby="sign-destination-help"
              {...register("destinationUrl")} />
            <p
              id="sign-destination-help"
      ...
      {token && <section className="space-y-2 rounded-xl border bg-background p-5 sm:p-6">
        ...
        <a
          href={getPublicSignUrl(token)}
          target="_blank"
          rel="noreferrer"
          className="block break-all text-sm text-primary hover:underline"
        >
          {getPublicSignUrl(token)}
        </a>
        <p className="text-sm text-muted-foreground">
          Conservarás este enlace y el mismo QR aunque cambies el destino.
        </p>
      </section>}
      <section className="flex items-center justify-between gap-6 rounded-xl border bg-background p-5 sm:p-6">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Al desactivarlo dejará de redirigir, conservando su QR.
          </p>
        </div>
        <Controller
          control={control}
