  ...
  return columnHelper.columns([
    ...
    columnHelper.display({
      ...
      cell: ({ row }) => <div className="flex items-center gap-2">
        <a href={getPublicSignUrl(row.original.token)} target="_blank" rel="noreferrer"
          className="max-w-40 truncate text-primary hover:underline">{getPublicSignUrl(row.original.token)}</a>
        <Button
          size="icon-sm"
          variant="ghost"
    ...
    columnHelper.accessor("destination_url", {
      header: "Destino",
      cell: ({ row }) => <span className="block max-w-40 truncate" title={row.original.destination_url ?? undefined}>
    ...
    columnHelper.display({
      ...
      cell: ({ row }) => {
        ...
        return <Badge variant="secondary" className={active ? "bg-green-100 text-emerald-700" : ""}>
    ...
    columnHelper.accessor("created_at", {
      header: "Fecha",
      cell: ({ row }) => new Intl.DateTimeFormat("es-MX", { dateStyle: "medium" }).format(new Date(row.original.created_at)),
    }),
    columnHelper.display({
      id: "actions", header: "Acciones",
      cell: ({ row }) => <div className="flex items-center gap-2">
        <Button size="sm" variant="outline" disabled={busyToken === row.original.token}
          onClick={() => onDownload(row.original)}><Download />QR SVG</Button>
        {canManage && <Button size="icon-sm" variant="ghost" nativeButton={false} render={
          <Link href={`/dashboard/signs/${row.original.id}/edit`} aria-label={`Editar ${row.original.label}`}><Pencil /></Link>
        } />}
      </div>,
    }),
  ])
