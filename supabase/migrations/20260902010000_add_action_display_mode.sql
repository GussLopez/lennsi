...
alter table public.actions
  drop constraint if exists actions_display_mode_check;

alter table public.actions
  add constraint actions_display_mode_check
  check (display_mode in ('link', 'icon'));

alter table public.actions
  drop constraint if exists actions_type_check;
    ...
    'actions', coalesce(
      (
        ...
        where action.restaurant_id = branch.restaurant_id
          ...
          and (
            (exists (
              select 1 from public.actions as branch_action
